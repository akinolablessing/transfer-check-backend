...
def extract_info(text: str) -> Dict[str, Any]:
    ...
    bank_search_text = recipient_section_match.group(1) if recipient_section_match else text
    bank_match = re.search(banks_pattern, bank_search_text, re.IGNORECASE)
    if bank_match:
...
def get_successful_transactions(db: Session, user_id):
    return db.query(Agent).filter_by(id=user_id).first().successful_transactions

def get_unsuccessful_transactions(db: Session, user_id):
    return db.query(Agent).filter_by(id=user_id).first().unsuccessful_transactions

def get_transactions(db: Session, user_id):
    return db.query(Agent).filter_by(id=user_id).first().transactions
